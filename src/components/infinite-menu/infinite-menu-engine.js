// WebGL2 engine for the InfiniteMenu component (React Bits).
// Kept in plain JS on purpose: the shader/matrix internals are ported verbatim
// from the upstream source so no transcription bug creeps in. The typed React
// boundary lives in infinite-menu.tsx; types for this module live in
// infinite-menu-engine.d.ts.
...
class ArcballControl {
  ...
  constructor(canvas, updateCallback) {
    ...
    // Bound handlers stored so dispose() can removeEventListener them (they'd
    // otherwise leak on every teardown/rebuild — canvas node can outlive the
    // control instance).
...
export class InfiniteGridMenu {
  ...
  // Added to upstream: stop the rAF loop so the component can tear down GPU work
  // when it unmounts (toggling back to the list view).
  stop() {
    if (this.#rafId !== null) {
  ...
  // Added to upstream: full teardown. Stops the loop, removes canvas pointer
  // listeners, and frees GL resources so repeated mount/unmount (toggling the
  // view, StrictMode's dev double-mount, or an items change) doesn't leak GPU
  // memory or stack duplicate listeners on the same canvas.
  dispose() {
    this.disposed = true
    this.stop()
    ...
    if (this.discBufferHandles) {
      this.discBufferHandles.forEach(
        (buffer) => buffer && gl.deleteBuffer(buffer),
      )
    }
  ...
  #init(onInit) {
    ...
    const discPositionBuffer = makeBuffer(
      gl,
      this.discBuffers.vertices,
      gl.STATIC_DRAW,
    )
    const discUvBuffer = makeBuffer(gl, this.discBuffers.uvs, gl.STATIC_DRAW)
    const { va: discVAO, indexBuffer: discIndexBuffer } = makeVertexArray(
      gl,
      [
        [discPositionBuffer, this.discLocations.aModelPosition, 3],
        [discUvBuffer, this.discLocations.aModelUvs, 2],
      ],
      this.discBuffers.indices,
    )
    this.discVAO = discVAO
    // Tracked so dispose() can free them (deleting the VAO does not free its buffers).
    this.discBufferHandles = [discPositionBuffer, discUvBuffer, discIndexBuffer]
  ...
  #initTexture() {
    ...
    Promise.all(
      ...
      // Bail if the instance was torn down while images were still loading —
      // otherwise we'd upload onto a deleted texture / disposed context.
